...
//             $res['duration']= $duration;
    ...
    protected function responseCountry($country)
    {
        $res["id"] = $country->id;
        $res["name"] = $country->name;
        return $res;
    }
    protected function responsehash($hash)
    {
        $res["id"] = $hash->id;
        $res["name"] = $hash->hashtag;
        return $res;
    }


    protected function responseCity($city)
    {
        $res["id"] = $city->id;
        $res["name"] = $city->name;
        return $res;
    ...
    protected function responseSubCity($subCity)
    {
        $res["id"] = $subCity->id;
        $res["name"] = $subCity->name;
        return $res;
    }

    protected function responseLocality($locality)
    {
        $res["id"] = $locality->id;
        $res["name"] = $locality->name;
        return $res;
    }
...
//            dd($request->tags);
            ...
            preg_match_all('/#([^\s]+)/', $spot->desc , $matches);
            $hashs = [];
            foreach ($matches[0] as $hashtag){
                ...
                foreach ($hashs as $tag) {
                    ...
                    if (!$tag_found) {
                        ...
                        $spot->hashs()->attach($newTag->id);
                    } else {
                        $spot->hashs()->attach($tag_found->id);
                    }
                }
            }
        ...
        if ($validatedData->passes()) {
            ...
            if ($spot->publisher_id == $request->publisher_id){
                ...
                $spot->update([
                    ...
                    'lat' => $request->lat,
                    'lng' => $request->lng,
                    'country_id' => ($request->has('country_name')) ? $country->id : $spot->country_id ,
                    'city_id' => ($request->has('admin_area'))  ? $city->id : $spot->city_id,
                    'subcity_id' => ($request->has('sub_adminArea')) ? $subcity->id : $spot->subcity_id,
                    'locality_id' => ($request->has('locality'))  ? $locality->id : $spot->locality_id,
                    'journal_id' => !is_numeric($request->journey_id) ? null : $request->journey_id,
                    'status' => 1,
            ...
            foreach ($matches[0] as $hashtag){
                ...
                foreach ($hashs as $tag) {
                    ...
                    if (!$tag_found) {
                        ...
                        $spot->hashs()->detach($tag_found->id);
                        $spot->hashs()->attach($tag_found->id);
                    }
                }
            }
...
//                 }
    ...
    public function  getSpot(Request $request)
    {
     ...
     public function  get_journy_spots(Request $request)
    {
        $validatedData = Validator::make($request->all(), [
            'publisher_id' => 'required|numeric|exists:publishers,id',
            'journey_id' => 'required|numeric|exists:spots,journal_id'
        ]);



        $spot = Spot::where("journal_id", $request->journey_id)->get();
    ...
    public function getSpotsCount(Request $request)
    {
        ...
        if ($validatedData->passes()) {
            ...
            if ($request->zoom <= 7) {
                ...
                $countrySpots = Country::whereHas('spots')->get();
    ...
    public function SearchedSpots(Request $request)
    {
        ...
        if ($validatedData->passes()) {
            ...
            }else if ($request->type == "locality") {
                $locality = Locality::where("id", $request->id)->first();
                if ($locality) {
                    $localitySpots = $locality->spots()->paginate(20);
                    return $this->apiResponse(["Spots" => SpotResource::collection($localitySpots)->foo($request->publisher_id)], null, 200);
                }
    ...
    public function editJourney(Request $request)
    {
         $validator = Validator::make($request->all(),[
            'name'                => 'nullable|string',
            'desc'                => 'nullable|string',
            'publisher_id'        => 'required|exists:publishers,id',
            'journey_id'          => 'required|exists:journals,id'
        ]);

        if ($validator->passes()){
            $journey = Journal::find($request->journey_id);
            if ($request->publisher_id != $journey->publisher_id)
            {
                $msg = $request['lang'] == 'ar' ? 'الرحله ليست خاصه بك' : ' you are not owner of journy';
                return response()->json(
                    [
        ...
        if($request->has('name')) {
         ...
         if($request->has('desc')) {
           ...
           if($journey->update())
           {
            ...
            $msg = $request['lang'] == 'ar' ? '  تم تعديل الرحله بنجاح' : ' Journy is updated successfully.';
  ...
  public function deleteJourney(Request $request)
    {
        ...
        if ($validator->passes()){
           ...
           if($journey->delete())
           {
            ...
            $msg = $request['lang'] == 'ar' ? ' تم حذف الرحله بنجاح' : ' Journy is deleted successfully.';
...
//                    'meta'   => $meta
    ...
    public function search(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            $user = User::where("display_name", "LIKE", "%" . $request['search_text'] . "%")->get();
            ...
            $country = Country::where("name", "LIKE", "%" . $request['search_text'] . "%")->get();

            $dataCountry = $country->map(function ($country) {
                return $this->responseCountry($country);
            });


            $city = City::where("name", "LIKE", "%" . $request['search_text'] . "%")->get();

            $dataCity = $city->map(function ($city) {
                return $this->responseCity($city);
            });

            $hash = Hash::where("hashtag", "LIKE", "%" . $request['search_text'] . "%")->get();

            $dataHash= $hash->map(function ($hash) {
                return $this->responsehash($hash);
            });


            $subCity = Subcity::where("name", "LIKE", "%" . $request['search_text'] . "%")->get();

            $dataSubCity = $subCity->map(function ($subCity) {
                return $this->responseSubCity($subCity);
            });

            $locality = Locality::where("name", "LIKE", "%" . $request['search_text'] . "%")->get();

            $dataLocality = $locality->map(function ($locality) {
                return $this->responseLocality($locality);
            });

            $spot = Spot::where("title", "LIKE", "%" . $request['search_text'] . "%")->get();

            return $this->apiResponse(["Spots" =>  SpotResource::collection($spot)->foo($request->publisher_id)
                , "Users" => $data
                , "Countries" => $dataCountry
                , "Cities" => $dataCity
                , "SubCities" => $dataSubCity
